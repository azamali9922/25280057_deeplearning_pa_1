...
I worked with a listing-level dataset where the goal is to predict \texttt{price\_class} (a multi-class label). I started by loading the CSV and inspecting the schema.
...
One of the first things I noticed is that the training data has quite a few missing values. Below is the breakdown:
...
The key columns with missing data are \texttt{neighbourhood\_group} ($\sim$800 nulls), \texttt{room\_type} ($\sim$600 nulls), \texttt{minimum\_nights} ($\sim$1300 nulls), and \texttt{amenity\_score} ($\sim$900 nulls). This becomes important again in Part D when I look at generalization.

\subsubsection{Handling Missing Values}

I handled the missing values using the following strategies:
...
Class 1 dominates at around 56\%, while Class 3 only has about 6\% of the samples. With this kind of imbalance, a naive model can get a deceptively high accuracy by over-predicting Class 1.
...
\texttt{(41348, 13)} --- 41,348 samples with 13 features (4 numeric + 5 \texttt{neighbourhood\_group} dummies + 4 \texttt{room\_type} dummies).
...
I implemented a feedforward neural network with two hidden layers (an MLP) using only NumPy. The architecture is:
...
Weights are initialized with small random values (scaled by 0.1) and biases are set to zero. I used batch gradient descent with a learning rate of 0.1 for 200 iterations.
...
\begin{figure}[H]
    ...
    \label{fig:hand_derivation_input_grads}
\end{figure}
...
Intuitively, if $\left|\frac{\partial L}{\partial x_i}\right|$ is large for some feature $x_i$, then a small change in $x_i$ would cause a noticeable change in the loss. In other words, the model is ``paying attention'' to that feature. On the other hand, if the gradient is close to zero, the model is relatively insensitive to that input, so changing it has little effect on the output.
...
\begin{itemize}
    ...
    \item \textbf{Chat link:} \url{https://gemini.google.com/share/46d121509b79}
...
\begin{itemize}
    ...
    \item \textbf{Usage --- Report writing:} Used VS Code Copilot Chat to help structure and write this  report, including formatting tables, filling in placeholder values from notebook outputs, and fixing compilation errors.
